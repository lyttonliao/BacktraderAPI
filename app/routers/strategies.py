...
@router.get(
    "/{strategy_id}", response_model=Strategy, dependencies=[Depends(JWTBearer())]
)
async def read_strategy(strategy_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
...
async def update_strategy_for_user(
    ...
    db_strategy = await strategies.update_user_strategy(
        db, strategy_params, strategy_id, user_id
    )
    return db_strategy
